...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['district_t_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/webcam_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/TegirmonDistrict/" + del_data.id, requestOptions);
          ...
          if(response.status == 201 || response.status == 200)
          {
            ...
            this.district_t_delete_row(index);
          }
          else{
