...
<div>
      ...
      <div class="tablePageLeft">
        ...
        <div class="SearchNameBarcode mt-1">
          ...
          <div class="FoundProductTable border" v-on:click.capture="focusBarcode">
            ...
            <div class=" px-3 py-2">
              <div class="row">
                <div class="col-3" v-for="(item,i) in productList" :key="i" @click="clickBarcodeData(item)">
                  <div class="border shadow p-2 mt-2" style="cursor:pointer;">
                    <div style="height:20px; overflow:hidden;">
                      <p class="m-0 mb-1" style="color:#707070"> {{item.product.name}}</p>
                    </div>
                    <p class="m-0 mb-0 text-right"> {{item.product.price}}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="SummaBtnRight border"  v-on:click.capture="focusBarcode">
        ...
        <div class="d-flex" v-on:click.capture="focusBarcode">
          <div class="w-50">
            ...
            <div class="text-center mt-1  linkMain" style="padding: 9.2px 0;" @click="pulOlibQolishChiqarNarsa">
              <span class="m-0 p-0 " >{{$t('pul_olish')}}</span>
            </div>
            <div class="text-center mt-1  linkMain" style="padding: 9.2px 0;" @click="rasxodClick">
          ...
          <div class="w-50">
            ...
            <mdb-btn color="warning" style="font-size:11px; padding: 12px 0;" @click="clickPay" class="m-0  mt-1 w-100"> {{$t('dolg_summ')}}</mdb-btn>
      ...
      <div class="acceptBox px-3 py-2">
        ...
        <div class="d-flex mt-3">
          <div class="skidka " style="width:35%">
            <div style="position:relative;">
              <input type="text" v-model="persantage_discount" @input="summaDrag" class="form-control form-control-sm border mt-2 text-right" style="border:none; outline:none;font-weight:bold;" >
        ...
        <div class="d-flex align-items-center mt-3" style="position:relative;">
          ...
          <div class="price ml-4 mt-2 border d-flex justify-content-end align-items-end"   style="width:58%; position:relative;">
            ...
            <input type="text" style="height:45px; width:100%; font-size:25px; border:none;" v-model="UnitSumString" class="px-2" @keyup="funcCurrency($event.target.value)">
            <small style="position:absolute; top:-16px; left:3px;">
                {{$t('')}}
              </small> 
          </div>
  ...
  <modal-train  :show="rasxod_show" headerbackColor="white"  titlecolor="black" :title="$t('change_product')" 
    @close="rasxod_show = false" width="50%">
      <template v-slot:body>
        <rasxod @close="rasxod_show = false" >
        </rasxod>
      </template>
  </modal-train>
  <modal-train  :show="pul_olib_qolish" headerbackColor="white"  titlecolor="black" :title="$t('pul_olish')" 
    @close="pul_olib_qolish = false" width="50%">
      <template v-slot:body>
        <chiqarPulOlish @close="pul_olib_qolish = false" >
        </chiqarPulOlish>
      </template>
  </modal-train>
