...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['device_delete_row']),
      for_edit(edit_data)
      {
        this.$router.push("/device_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ArchiveDevices/" + del_data.id, requestOptions);
