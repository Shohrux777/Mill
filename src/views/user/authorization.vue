...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(localStorage.UserIdForAuth > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/ArchiveUser/' + localStorage.UserIdForAuth);
    ...
    async mounted(){
      ...
      this.dept_id = data.archiveDepartmentid;
      this.phone_number = data.position;
    },
