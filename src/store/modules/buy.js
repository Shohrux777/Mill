export default {
    ...
    actions: {
        async fetchBuy(ctx) {
            const res = await fetch(ctx.rootState.hostname + '/TegirmonInvoice/getPaginationBugdoyPrixodQilinganTovarlarListiVaqtsz?page=0&size=1000');
    ...
    mutations: {
        updateBuy(state, data) {
            console.log(data)
            state.m_buy.rows = data.items_list;
        },
